...
run.1.path <- file.path("output", "main_P3")
run.2.path <- file.path("output", "main_P4")
...
run.stats <- bind_rows(run.0.stats, run.1.stats, run.2.stats)
...
dunn_plot <- dunn_table %>%
  ...
  ggplot(aes(x=dunn_all_0, y=dunn_all_1, size=n_samples, col=outliers)) + 
  geom_point() + 
  geom_abline(slope = 1) + 
...
comparison.summary %>%
  write_tsv(file = file.path(out.dir, "pathway.comparison.summary.tsv"))
